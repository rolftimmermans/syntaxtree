...
class BlockTest < MiniTest::Unit::TestCase
  ...
  test "call with block without parameters should return block" do
    assert { statement("foo(a, b) { puts x }").block.kind_of? SyntaxTree::Ruby::Block }
  end

  test "call with block without parameters should return block with param list" do
    assert { statement("foo(a, b) { puts x }").block.parameters.kind_of? SyntaxTree::Ruby::ParameterList }
  end
  ...
  test "call with block should return block" do
    assert { statement("foo(a, b) { |x| puts x }").block.kind_of? SyntaxTree::Ruby::Block }
  end

  test "call with block should return block with param list" do
    assert { statement("foo(a, b) { |x| puts x }").block.parameters.kind_of? SyntaxTree::Ruby::ParameterList }
  end

  test "call with block should return block with param list with param" do
    assert { statement("foo(a, b) { |x| puts x }").block.parameters.first.kind_of? SyntaxTree::Ruby::Identifier }
  end
  ...
  test "call with block with default param should return block with param list" do
    assert { statement("foo(a, b) { |x, y = 3| puts x }").block.parameters.kind_of? SyntaxTree::Ruby::ParameterList }
  end

  test "call with block with default param should return block with param list with param" do
    assert { statement("foo(a, b) { |x, y = 3| puts x }").block.parameters.last.kind_of? SyntaxTree::Ruby::DefaultParameter }
  end

  test "call with block with default param should return block with param list with param with identifier" do
    assert { statement("foo(a, b) { |x, y = 3| puts x }").block.parameters.last.identifier.kind_of? SyntaxTree::Ruby::Identifier }
  end
  ...
  test "call with block with only a default param should return block with param list with param" do
    assert { statement("foo(a, b) { |x = 3| puts x }").block.parameters.first.kind_of? SyntaxTree::Ruby::DefaultParameter }
  end

  test "call with block with only a default param should return block with param list with param with identifier" do
    assert { statement("foo(a, b) { |x = 3| puts x }").block.parameters.first.identifier.kind_of? SyntaxTree::Ruby::Identifier }
  end

  # Splat parameters
  test "call with block with splat param block should return block with param list with splat param" do
    assert { statement("foo(a, b) { |x, *other| puts x }").block.parameters.last.kind_of? SyntaxTree::Ruby::SplatParameter }
  end

  test "call with block with splat param block should return block with param list with splat param with identifier" do
    assert { statement("foo(a, b) { |x, *other| puts x }").block.parameters.last.identifier.kind_of? SyntaxTree::Ruby::Identifier }
  end
  ...
  test "call with block with block param block should return block with param list with block param" do
    assert { statement("foo(a, b) { |x, &block| puts x }").block.parameters.last.kind_of? SyntaxTree::Ruby::BlockParameter }
  end

  test "call with block with block param block should return block with param list with block param with identifier" do
    assert { statement("foo(a, b) { |x, &block| puts x }").block.parameters.last.identifier.kind_of? SyntaxTree::Ruby::Identifier }
  end
  ...
  test "call with empty block should return block with statement list" do
    assert { statement("foo(a, b) { }").block.statements.kind_of? SyntaxTree::Ruby::Statements }
  end
  ...
  test "call with block should return block with statement list" do
    assert { statement("foo(a, b) { foo\nbar; baz }").block.statements.kind_of? SyntaxTree::Ruby::Statements }
  end

  test "call with block should return block with statements" do
    assert { statement("foo(a, b) { foo\nbar; baz }").block.statements.first.kind_of? SyntaxTree::Ruby::Identifier }
  end
